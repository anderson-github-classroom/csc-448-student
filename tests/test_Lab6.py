...
def test_exercise_1():
    ...
    assert np.all(Lab6.to_adj(answer) == answers['answer_exercise_1'])
...
def test_exercise_3():
    ...
    assert np.all(Lab6.to_adj(answer) == answers['answer_exercise_3'])
    
def test_exercise_4():
    answer,discard = Lab6.modified_suffix_trie("panamabananas$")
    assert np.all(Lab6.to_adj(answer) == answers['answer_exercise_4'])
    
def test_exercise_5():
    answer = Lab6.suffix_tree_construction("panamabananas$")
    assert np.all(Lab6.to_adj(answer) == answers['answer_exercise_5'])
